...
if (USARTx == USART1)	//Tx 
	{
		...
		DMA_STM_F4.DMA_Stream = DMA2_Stream7;//выбор потока ДМА напр. -  DMA2_Stream0
		DMA_STM_F4.DMA_Interrupt =  DMA2_Stream7_IRQn;// прерывание из stm32f411xe.h, например - DMA2_Stream0_IRQn
...
DMA_STM_F4.DMA_Peripheral_address = (volatile uint32_t*)&(USART_def->DR);//адрес перефирии
...
if (USARTx == USART1)	//Rx 
	{
		DMA_STM_F4.DMA_Number = DMA2;//выбор ДМА, напр. - DMA2
		DMA_STM_F4.DMA_Stream = DMA2_Stream5;//выбор потока ДМА напр. -  DMA2_Stream0
		DMA_STM_F4.DMA_Interrupt =  DMA2_Stream5_IRQn;// прерывание из stm32f411xe.h, например - DMA2_Stream0_IRQn
		DMA_STM_F4.DMA_Chanel =  4;//выбор канала ДМА
	}		
...
DMA_STM_F4.DMA_Peripheral_address = (volatile uint32_t*)&(USART_def->DR);//адрес перефирии, например (volatile uint32_t*)&(USART2->DR);
